...
def list_all_input_file(input_dir):
	...
	onlyfile = [f for f in os.listdir(input_dir) if (os.path.isfile(os.path.join(input_dir,f)) and os.path.splitext(f)[1] == ".txt")]
	return onlyfile

def save_preprecessing_data(Mi_data):
	with open(ouput_dir+ouput_file,'w') as wf:
		for i,each_line in enumerate(Mi_data['timestamp']):
			wf.write(each_line,Mi_data['square_id_1'],Mi_data['square_id_2'],Mi_data['interaction'])
...
def combine_data(Mi_data,Mi_data_proceesed):
	...
	for i,each_line in enumerate(Mi_data['timestamp']):
		...
		if end_time <= date_time + timedelta(minutes = 10 ) :
				...
				Mi_data_proceesed['sms_in_activity'].append(sms_in_activity/(time_interval/10))
				Mi_data_proceesed['sms_out_activity'].append(sms_out_activity/(time_interval/10))
				Mi_data_proceesed['call_in_activity'].append(call_in_activity/(time_interval/10))
				Mi_data_proceesed['call_out_activity'].append(call_out_activity/(time_interval/10))
				Mi_data_proceesed['internat_traffic_activity'].append(temp_activity/(time_interval/10))
...
def load_data_from_file(file_path):
	...
	with open(file_path,'r') as f:
		...
		for line in f.readlines():
			...
			sms_in_activity = float(split_line[3].strip()) if split_line[3].strip() else 0
			sms_out_activity = float(split_line[4].strip()) if split_line[4].strip() else 0
			call_in_activity = float(split_line[5].strip()) if split_line[5].strip() else 0
			call_out_activity = float(split_line[6].strip()) if split_line[6].strip() else 0
			internat_traffic_activity = float(split_line[7].strip()) if split_line[7].strip() else 0
	...
	output_filename = 'output_'+ os.path.splitext(os.path.basename(file_path))[0]
